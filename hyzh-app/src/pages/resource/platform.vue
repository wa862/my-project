...
// 数据库数据
const nationalDatabases = ref<DatabaseItem[]>([])
const regionalDatabases = ref<DatabaseItem[]>([])
const surveyDatabases = ref<SurveyItem[]>([])

// 图片URL处理
const getImageUrl = (imageUrl: string | undefined) => {
  if (!imageUrl) return '/static/images/default-db.png'
...
const loadNationalData = async () => {
  try {
    ...
    // 使用uni.request调用API，与网页版保持一致
    const [nationalRes, regionalRes] = await Promise.all([
      uni.request({
        url: `${API_BASE_URL}/api/resources`,
        data: { category_type: 'national', pageSize: 20 },
        method: 'GET'
      }),
      uni.request({
        url: `${API_BASE_URL}/api/resources`,
        data: { category_type: 'regional', pageSize: 20 },
        method: 'GET'
      })
    ])
    
    // 处理响应数据，与网页版保持一致
    const nationalData = nationalRes.data as ApiResponse<DatabaseItem[]>
    const regionalData = regionalRes.data as ApiResponse<DatabaseItem[]>
    
    if (nationalData?.success) {
      nationalDatabases.value = nationalData.data || []
    }
    
    if (regionalData?.success) {
      regionalDatabases.value = regionalData.data || []
    }
  } catch (error) {
    console.error('加载国家数据库数据失败:', error)
    // 使用回退数据
    nationalDatabases.value = [
      {
        id: 1,
        title: '年度数据',
        description: '国家统计局年度数据',
        url: 'https://data.stats.gov.cn/easyquery.htm?cn=C01',
        image_url: '/static/images/default-db.png'
      },
      {
        id: 2,
        title: '各部门数据',
...
const loadUniversityData = async () => {
  try {
    ...
    // 处理响应数据，与网页版保持一致
    const resData = res.data as ApiResponse<SurveyItem[]>
    
    if (resData?.success) {
      surveyDatabases.value = resData.data || []
    } else {
