<template>
  <div class="research-platform-management">
    <h2 class="page-title">
    ...
    >
      ...
      <el-table-column prop="title" label="平台名称" width="200" show-overflow-tooltip />
      <el-table-column prop="description" label="平台描述" width="300" show-overflow-tooltip />
      <el-table-column label="平台图片" width="120">
      ...
      <el-table-column prop="link" label="平台链接" width="200">
        <template #default="{ row }">
    ...
    >
      ...
      >
        ...
        <el-form-item label="平台链接" prop="link">
          <el-input v-model="form.link" placeholder="请输入平台URL" />
        </el-form-item>
...
const api = axios.create({
  baseURL: 'http://localhost:3000/api/research-platforms',
  timeout: 10000,
  headers: {
...
const platformList = ref<ResearchPlatform[]>([])
...
const fetchData = async () => {
  ...
  try {
    ...
    ElMessage.error(error.response?.data?.message || error.message || '获取科研平台数据失败')
  } finally {
    loading.value = false
  }
}
...
const handleEdit = (row: ResearchPlatform) => {
...
const handleDelete = async (row: ResearchPlatform) => {
  try {
